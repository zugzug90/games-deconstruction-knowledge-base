...
`TL;DR` - версия документа представляет собой секцию **[Выводы, гипотезы, мысли от себя](#выводы-гипотезы-мысли-от-себя)**.

>
> ![TG Group QR](img/tg_qr.png)
>
...
## Оглавление
- [Введение](#введение)
- [Изученные игры](#изученные-игры)
- [Особенности реализации](#особенности-реализации)
    - [Экраны уровня](#экраны-уровня)
    - [Базовые фичи](#базовые-фичи)
    - [Расстановка предметов](#расстановка-предметов)
- [Монетизация](#монетизация)
- [Визуальные эффекты и flow поиска](#визуальные-эффекты-и-flow-поиска)
    - [Правильно найденный предмет](#правильно-найденный-предмет)
    - [Миссклик](#миссклик)
- [Звуки и музыка](#звуки-и-музыка)
- [Технические аспекты](#технические-аспекты)
    - [Обработка кликов](#обработка-кликов)
    - [Сохранение прогресса](#сохранение-прогресса)
- [Прогрессия](#прогрессия)
    - [Hidden Objects Games](#hidden-objects-games)
    - [Hidden Objects - The Journey](#hidden-objects---the-journey)
- [Выводы, гипотезы, мысли от себя](#выводы-гипотезы-мысли-от-себя)
...
### Экраны уровня
При изучении HOG игр с Google Play, т.е. поддерживающих только мобильную портретную ориентацию были обнаружены следующие размеры уровня в абсолютных единицах (пиксели) для моего устройства:
...
#### Веб-версия 
В Веб версии игры уровень размещается по центру, а по краям добавляются декоративные шторки
...
Основные моменты, приёмы и трюки, задающие челлендж, усложняющие поиск предметов:
1. Предметы варьируются по размерам: от больших и легко находимых (типа бочек, статуй, людей в форме, фонарных столбов) до мелких и мелко-детализированых (курительные трубки, бутылки, фишки, розы, стаканы с коктейлями)
2. Размещение предметов на окружении тех же оттенков и цветов. Например, красная шляпа на красном столе.
3. Размещение предметов в группе похожих предметов окружения, но не являющихся интерактивными. Например, размещение бутылки в горе мусора из других бутылок или емкостей.
4. Анимирование окружения - усложняет поиск, отвлекает, вносит шум и даёт большую нагрузку на внимание игрока.
5. Размещение мелкодетализированного предмета на мелкодетализированном окружении или текстуре. Пример: статуи гаргулий, ангелов на фоне колонн и рельефных стен Собора Парижской богоматери.
6. Близкий двойник-фейк. Предметы-обманки, максимально близко похожие на искомые из ростера предметы, но отличающиеся 1-2 незначительными деталями. Пример: бутылка в руке отдыхающего в кафе мужчины, очень похожа на предмет бутылки, но слегка отличается формой.
...
## Визуальные эффекты и flow поиска
...
4. Есть звуки найденного предмета - при нажатии короткий звук (например, pop, пуньк, т.е. не раздражающий, а максимально приятный), это дополнительный аудио-фидбек к визуальному. В игре с поиском 1 типа предметов - "Find The Cat - Spot It!" - каждый найденный котик задорно мяукает, с вариациями мяуканья.
...
Можно попробовать такие теги и ключевые слова, если пытаться генерировать музыку промтами через Suno: `jazz`, `soothing`, `earworm`, `chill`, `cozy`, `relaxing`.
...
То же самое было обнаружено и в мобильной игре "Hidden Objects - The Journey". См. пруф гифку ниже (белый кружок - тач игрока):
...
Два "уровня" сохранения - 1. сохраняются пройденные игроком уровни; 2. сохраняется прогресс уровня по мере открытия предметов

Прогресс сохраняется после каждого найденного на уровне предмета. 
Таким образом, после выхода из уровня и возврата в него - игрок не теряет прогресс, а уже имеет уровень с частично открытыми им предметами. 

В то же время однако и сохраняются все остальные его показатели:

- количество попыток
- количество оставшихся повер-апов для использования.

## Прогрессия

Ниже представлены таблички сложности уровней для исследованных игр:

### Hidden Objects Games

| Уровень | Типов предметов | Всего | Найти штук каждого типа |
|---------|-----------------|-------|-------------------------|
| 1       | 4               | 32    | 5 - 10                  |
| 2       | 12              | 316   | 21 - 31                 |
| 3       | 12              | 284   | 21 - 25                 |
...
__Дополнительно к кору игры:__

- В данной игре ещё есть фишка по сбору уникальных редких предметов. На каждом уровне он ровно один и очень хорошо спрятан. Находить его необязательно, но это дополнительный мощный завлекающий элемент, заставляющий игрока провести больше времени на каждом уровне в попытках найти его. Глобально есть некая отдельная карта-уровень в отрыве от остальных, где игрок собирает полную коллекцию этих уникальных предметов (всего нужно собрать 398 штук на текущий день - 2025-08-17).
   
   Выглядит этот экран вот так:
   
   ![Экран Коллекции](img/unique_items_collection_hog.jpg)
    
- Также в этой игре все уровни начиная со второго имеют дополнительные уровни сложности, которые игрок может закрыть - меняется расстановка и ростер предметов, увеличивается количество, которое нужно найти, уровни: легко, средний, сложно, эксперт. После всех их прохождений открывается ещё один с неким секретным предметом.
...
Как видим, здесь как минимум на первых уровнях авторы игры решили пойти путем фиксированных ростеров предметов и их количественного распределения и суммарного размещения - 60 штук.
